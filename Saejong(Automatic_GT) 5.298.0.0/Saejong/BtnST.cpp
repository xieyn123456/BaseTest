...
CButtonST::CButtonST()
{
	...
	m_ForeColor         =RGB(230,230,230);
	m_FocuseColor       =RGB(230,230,230);
	m_BkColor           =RGB(230,230,230);
	p_Font              =NULL;
	m_SideColor         =RGB(255,255,255);
...
/*                                                                      */
/************************************************************************/
HBITMAP CButtonST::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor,UINT unLeftTop,UINT unRightButtom)
{
	HBITMAP		hMask		= NULL;
	HDC			hdcSrc		= NULL;
	HDC			hdcDest		= NULL;
	...
	hMask = ::CreateBitmap(dwWidth, dwHeight, 1, 1, NULL);
//	hMask = ::CreateBitmap(dwWidth, dwHeight, 0, 0, NULL);
	if (hMask == NULL)	return NULL;
	...
	::BitBlt(hdcDest,0,0, dwWidth, dwHeight, hdcSrc, unLeftTop, unRightButtom,SRCCOPY );

	//crSaveDestText = ::SetTextColor(hdcSrc, RGB(255, 255, 255));
	//::SetBkColor(hdcSrc,RGB(0, 0, 0));

	//::BitBlt(hdcSrc,10, 50, dwWidth, dwHeight, hdcDest, unLeftTop, unRightButtom, MERGEPAINT);
...
/*                                                                      */
/************************************************************************/
DWORD CButtonST::SetBitmapsCut(int nBitmapIn, COLORREF crTransColorIn,UINT unLeftTop,UINT unRightBottom)
{
	HBITMAP		hBitmapIn		= NULL;
	HINSTANCE	hInstResource	= NULL;
	...
	return SetBitmapsCut(hBitmapIn, crTransColorIn,unLeftTop,unRightBottom);
   
}
/************************************************************************/
/*                                                                      */
/************************************************************************/
DWORD CButtonST::SetBitmapsCut(HBITMAP hBitmapIn, COLORREF crTransColorIn, UINT unLeftTop/* =0 */,UINT unRightBottom/* =0 */)
{
	int		nRetValue;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		m_csBitmaps[0].dwWidth = 90;//图片尺寸
       m_csBitmaps[0].dwHeight = 110;

		// Create mask for bitmap In
		m_csBitmaps[0].hMask = CreateBitmapMask(hBitmapIn, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight, crTransColorIn,unLeftTop,unRightBottom);
		if (m_csBitmaps[0].hMask == NULL)
		{
			FreeResources();
			return BTNST_FAILEDMASK;
...
DWORD CButtonST::SetDefaultColors(BOOL bRepaint)
{
	...
	m_crColors[BTNST_COLOR_BK_IN]		= RGB(0,255,0);
	m_crColors[BTNST_COLOR_FG_IN]		= ::GetSysColor(COLOR_BTNTEXT);
	m_crColors[BTNST_COLOR_BK_OUT]		= RGB(0,255,0);
	m_crColors[BTNST_COLOR_FG_OUT]		= ::GetSysColor(COLOR_BTNTEXT);
	m_crColors[BTNST_COLOR_BK_FOCUS]	= RGB(0,255,0);
	m_crColors[BTNST_COLOR_FG_FOCUS]	= ::GetSysColor(COLOR_BTNTEXT);
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, LPCRECT pRect)
{
	...
	if (m_bMouseOnButton && (! m_bIsPressed))
				...
				crColor = m_crColors[BTNST_COLOR_BK_FOCUS];
				  ...
				  crColor = m_crColors[BTNST_COLOR_FG_OUT];
	...
	/**////second code
	if (m_bMouseOnButton && (! m_bIsFocused))
		crColor = RGB(255,255,255);
	else
	{
		if (m_bIsFocused)
	...
	if (m_bMouseOnButton)
	{
		m_BkColor=m_FocuseColor;
	}
	else
	{
		m_BkColor=m_ForeColor;
		m_Press=false;
	}
	...
	CBrush		brBackground; 
	brBackground.CreateSolidBrush(m_BkColor);
	pDC->SelectObject( &brBackground );
	pDC->RoundRect(pRect,CPoint(0,0));	//画圆角矩形
	
	CPen Pen;
	crColor = m_SideColor;
//	crColor = RGB(28, 79, 94);//20130214/新增加在Win8背景 為按下的按鈕不用顯示邊框  
	Pen.CreatePen(PS_SOLID, 1, crColor );
	pDC->SelectObject( &Pen );
	pDC->RoundRect(pRect,CPoint(0,0));
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, LPCRECT pRect)
{
	...
	else
	{
		...
		{
			m_Press=true;
			Pen.CreatePen(PS_SOLID, 1, m_FocuseColor );
			pDC->SelectObject( &Pen );
			pDC->RoundRect(pRect,CPoint(1,1));
		}	
		else
		{
			m_Press=false;
...
void CButtonST::SetBkColor(COLORREF color)
{
...
void CButtonST::SetFocuseColor(COLORREF color)
{
	m_FocuseColor = color;
	Invalidate();
}
...
void CButtonST::SetText(CString str)
{
	SetWindowText(str);
}

void CButtonST::SetTextFont(int FontHight, LPCTSTR FontName)
{
...
void CButtonST::SetTextFontEx(long FontHight, long FontWeight, LPCTSTR FontName)
{
//	LOGFONT* log_font=NULL;
//	if ( p_Font )	delete p_Font;	//删除旧字体
//	p_Font = new CFont;
//	p_Font->GetLogFont(log_font);
//	log_font->lfWeight=FontWeight;
//	log_font->lfHeight=FontHight;
//	strcpy(log_font->lfFaceName,FontName);
//	p_Font->CreateFontIndirect(log_font);  
...
void CButtonST::SetSideColor(COLORREF color)
{
	m_SideColor=color;
	Invalidate();
}
